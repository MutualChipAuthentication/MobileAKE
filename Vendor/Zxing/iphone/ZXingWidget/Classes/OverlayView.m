...
//  //     UIButton * butt = [UIButton alloc] initWithFrame:CGRectMake(
...
-(void) addNavigationBar{
    CGFloat widthNavBar = 320.0;
    CGFloat xCenterButton = 270;
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        widthNavBar = 768.0;
        xCenterButton = 720.0;
    }
    UINavigationBar * navigationBar = [[[UINavigationBar alloc] initWithFrame:CGRectMake(0, 0, widthNavBar, 49)] autorelease];
    [navigationBar setBackgroundColor:[UIColor blackColor]];
    [navigationBar setBackgroundImage:[[UIImage alloc] init] forBarMetrics:UIBarMetricsDefault];
    
    UIButton * cancelButtonModal = [[[UIButton alloc] initWithFrame:CGRectMake(0.0, 0, 77.0, 33)] autorelease];
    [cancelButtonModal setBackgroundImage:[UIImage imageNamed:@"modal_anuluj.png"] forState:UIControlStateNormal];
     [cancelButtonModal setBackgroundImage:[UIImage imageNamed:@"modal_anuluj_active.png"] forState:UIControlStateHighlighted];
    [cancelButtonModal setTitle:NSLocalizedString(@"cancel", nil) forState:UIControlStateNormal];
    [cancelButtonModal setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    
    [[cancelButtonModal titleLabel] setFont:[UIFont fontWithName:@"TrebuchetMS-Bold" size:13]];
    [cancelButtonModal addTarget:self action:@selector(cancel:) forControlEvents:UIControlEventTouchUpInside];
    cancelButtonModal.center = CGPointMake(xCenterButton, 24);
    [navigationBar addSubview:cancelButtonModal];
...
- (void)drawRect:(CGRect)rect inContext:(CGContextRef)context {
    ...
    */
    ...
    searchFrame = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"qr.png"]];
    searchFrame.center = CGPointMake(self.window.screen.bounds.size.width/2, self.window.screen.bounds.size.height/2 );
